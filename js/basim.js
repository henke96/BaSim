...
const HTML_CALL_NUMBER = "callnumber";
...
function simInit() {
	...
	simCallNumberSpan = document.getElementById(HTML_CALL_NUMBER);
...
function simReset() {
	...
	simCurrentFoodId = 0;
	simTickCounter = 0;
	simTickCountSpan.innerHTML = simTickCounter;
	baInit(0, 0, "");
	plInit(-1, 0);
	simDraw();
}
			...
			for (let i = 0; i < itemZone.length; ++i) {
				...
				if (plX === item.x && plY === item.y) {
					if (pickupI === null) {
						...
						if (item.foodType() !== itemZone[pickupI].foodType()) {
							pickupI = i;
						}
					}
				}
			}
			if (pickupI !== null) itemZone.splice(pickupI, 1);
			plShouldPickupFood = false;
		}
		if (plPathQueuePos > 0) {
...
fFood.prototype.foodType = function() {
	let goodType = this.dropCall % 2;
	return this.isGood ? goodType : 2;
}
...
ruRunner.prototype.print = function(string) {
	console.log(simTickCounter + ": Runner " + this.id + ": " + string);
	this.chat = string;
}
var ruSniffDistance;
...
function baInit(maxRunnersAlive, totalRunners, maxHealersAlive, totalHealers, runnerMovements) {
	...
	baCallNumber = 0;
	simCallNumberSpan.innerHTML = baCallNumber;
...
function baTick() {
	baCallNumber = 1 + Math.trunc((simTickCounter - 2) / 50); // TODO
	simCallNumberSpan.innerHTML = baCallNumber;
	...
	if (simTickCounter > 1 && simTickCounter % 10 === 1) {
